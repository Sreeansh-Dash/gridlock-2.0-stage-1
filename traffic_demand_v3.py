...
def get_oof_aggregations(train_df, test_df, groupby_cols, target_col='demand', stat_funcs=['mean', 'std', 'median', 'count'], n_splits=5):
    """
    Compute Out-Of-Fold target aggregations for train_df to prevent leakage,
    and compute full-train aggregations for test_df.
    """
    kf = KFold(n_splits=n_splits, shuffle=True, random_state=SEED)
    
    # Initialize output columns in train_df
...
def engineer_features(train, test):
    ...
    # Split back to train and test for OOF Aggregations
...
def evaluate_models(X_train, y_train, X_val, y_val):
    print("Evaluating models on 20% holdout...")
    lgb_params = {
...
def train_final_and_predict(X_full, y_full, X_test, lgb_params, xgb_params, cat_params):
    print("\nRetraining models on FULL 100% dataset...")
    # LightGBM full
    # Since we can't use early stopping without a validation set, we use a fixed number of estimators
    # based on typical early stopping rounds from validation, e.g., 800.
...
def main():
    ...
    lgb_params, xgb_params, cat_params = evaluate_models(X_train_val, y_train_val, X_val, y_val)
    
    preds_test = train_final_and_predict(X_full, y_full, X_test, lgb_params, xgb_params, cat_params)
